...
describe('MultipartGenerator', () => {
  ...
  describe('generateMessage()', () => {
    ...
    (hasFormData && !isSafari ? it : it.skip)('creates ArrayBuffer', async () => {
      const message = await element.generateMessage();
      assert.typeOf(message, 'ArrayBuffer');
    });

    (hasFormData && !isSafari ? it : it.skip)('sets the contentType', async () => {
      await element.generateMessage();

      assert.typeOf(element.contentType, 'string');
    });

    (hasFormData && !isSafari ? it : it.skip)('sets the boundary', async () => {
      await element.generateMessage();
  ...
  describe('generatePreview()', () => {
    ...
    (hasFormData && !isSafari ? it : it.skip)('returns a string', async () => {
